"""Create reference workload schema under version control."""
...
def upgrade() -> None:
    op.execute(
        ...
        SELECT symbol, price_time, close,
        ...
        WITH ranked AS (
            SELECT symbol, price_time, close, volume,
                   ROW_NUMBER() OVER (PARTITION BY symbol ORDER BY price_time DESC) AS rn
            FROM stock_prices
            WHERE close IS NOT NULL
        )
        SELECT cur.symbol, cur.price_time, cur.close, prev.close AS previous_close,
...
def downgrade() -> None:
    op.execute(
        ...
        DROP VIEW IF EXISTS stock_daily_returns;
        DROP TABLE IF EXISTS lean_backtest_stage_timings;
        DROP TABLE IF EXISTS lean_backtest_runs;
        DROP TABLE IF EXISTS observability_runs;
